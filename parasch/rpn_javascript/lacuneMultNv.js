...
function construitTexte(texteTab)
{
	...
	for (var i=0, l = texteTab.length;i<l;i++)
	{
		...
		else if (texteTab[i].lastIndexOf("|")==(texteTab[i].length-1))
		{
			...
			var span = document.createElement("span");
			span.className="phg";
			var spanText = document.createTextNode(texteTab[i].substring(0,texteTab[i].length-1));
			if(texteTab[i].indexOf("|")==0)
			{
				spanText = document.createTextNode(texteTab[i].substring(1,texteTab[i].length-1));
			}
			span.appendChild(spanText);
			p.appendChild(span);
		}
