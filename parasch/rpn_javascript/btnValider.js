...
function check()
{
	...
	{
		for (var n=0, nbMax=solutions.length; n < nbMax; n++)
		{
			analyseCheck(n);
		}
		...
		var nbErreurs = (solutions.length-nbJustes);
		
		if (nbJustes==solutions.length || (nbEssais==3 && nbErreurs>0))
		{
			montrerSolutions();
			if(dernierExe)
			{
		...
		else
		{
			...
			{
				message = message+'Tu as répondu correctement à une question sur '+solutions.length+'.';
			}
			else
			{
				message = message+'Tu as répondu correctement à '+nbJustes+' questions sur '+solutions.length+'.';
			}
		}
