...
function construitLigne(no)
{
	...
	if (avecNo)
	{
		li0.innerHTML = (no+1)+". "+phrases[no];
	}
	else
	{
		li0.innerHTML = phrases[no];
	}
	...
	for (var noC = 1; noC<nbColonnes; noC++)
	{
		li[noC] = document.createElement("li");
...
function analyseCheck(no)
{
	...
	for (var i = 0; i < inputsLength; i++)
	{
		liParent[(nbColonnes-1)*no+i].className = "vide";//Initialisation pour faire disparaître la zone rouge
		
		if (nbEssais>2 && compteCheck==0)
		{
			liParent[(nbColonnes-1)*no+i].className = "faux";
		}
		else
		{
			if (nbEssais>2 && reponses[(nbColonnes-1)*no+i]!=solAdaptes[(nbColonnes-1)*no+i] && reponses[(nbColonnes-1)*no+i]!=0)
			{
				liParent[(nbColonnes-1)*no+i].className = "faux";
			}
		}	
		if (nbEssais>2 && reponses[(nbColonnes-1)*no+i]==solAdaptes[(nbColonnes-1)*no+i] && reponses[(nbColonnes-1)*no+i]!=0)
		{
			liParent[(nbColonnes-1)*no+i].className = "juste";
		}
	}
}
...
function montrerSolutions()
{
	for (var no=0, nbMax=solutions.length; no < nbMax; no++)
	{
		...
		for (var i = 0; i < inputsLength; i++)
		{
			if (solAdaptes[(nbColonnes-1)*no+i]!=0)
			{
				liParent[(nbColonnes-1)*no+i].className = "juste";
