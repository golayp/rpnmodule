...
//	fondRepMc9._visible=false;
	...
	var resultatAncienneCaseStr:String = resultatOperation(choixOperation, maCase[numeroAncienneCase].nb1, maCase[numeroAncienneCase].nb2).indexOf("pas");
	//resultatAncienneCaseStr = resultatAncienneCaseStr.split(" ").join("");
	//resultatAncienneCaseStr = resultatAncienneCaseStr.split("\\").join("");
	//resultatAncienneCaseStr = resultatAncienneCaseStr.split("n").join("");
	trace ("resultatAncienneCase:" +resultatAncienneCaseStr);
	trace("maCase[numeroNouvelleCase].conteneur.carre.val1.text" +maCase[numeroNouvelleCase].conteneur.carre.val1.text );
	trace("maCase[numeroNouvelleCase].conteneur.carre.val1.text==resultatAncienneCaseStr" +(maCase[numeroNouvelleCase].conteneur.carre.val1.text.indexOf("pas")==resultatAncienneCaseStr) );
	...
	switch (true)
	{
		...
		break;
		case(resultatAncienneCase==Number(maCase[numeroNouvelleCase].conteneur.carre.val1.text) || resultatAncienneCaseStr==maCase[numeroNouvelleCase].conteneur.carre.val1.text.indexOf("pas") )://Si on a cliqué sur un nb premier plus petit,dans l'ordre décroissant c'est juste. On met juste dans le tableau des réponses
...
{
	trace("On est dans preparationRetour");
	if (parcours[0]==""){
		_level0.rep_juste = false;
