...
//fondMc.fenetre_Mc.fond_Choix_1_Mc.guideTxt.embedFonts=true;
...
//fondMc.fenetre_Mc.fond_Choix_2_Mc.nonguideTxt.embedFonts=true;
...
function selectionne(caseActuelleX,caseActuelleY)
{
	...
	switch (true)
	{
		...
		case(numeroCaseActuelle==noCase(arriveeX,arriveeY))://Si on clique su la case arrivée, on rend toutes les cases inctives et la couleur d'arrivée est la couleur de après clique	 
			...
			trace("case arrivee dans selectionne"+ maCase[numeroCaseActuelle])
		...
		break;
		case(parcours.length == 0)://On a cliqué sur début (tant qu'on ne clique pas sur la case de départ, les suivantes ne sont pas clicables) et on met les coordonnées de départ dans parcours			
			parcours.push([caseActuelleX, caseActuelleY]);
			...
			for (p = 0; p < parcours.length-1; p++)
				{
					...
					if (parcours[p].toString()==tableauCoordonnees.toString())
					{
						...
						for (m = 1; m <=parcoursTemp.length; m++)
						{
							...
							//var monNoF = 1200+CaseDesactivee;
							var monNoM = avantCaseDesactivee;
							trace("caseDesactivee"+caseDesactivee)
							...
							trace("case annulee dans selectionne"+ maCase[caseDesactivee])
