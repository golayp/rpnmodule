	...
	bouton1n.onRelease=function()
		{
			...
			for (m = 0; m < tri.instances; m++) {
				...
				if (monUnite[elementsTires[m]].categorieSac == categorieDuSac) {
					nbEle++;
					trace("nbEle%4"+nbEle%4)
					monUnite[elementsTires[m]].visibilite(true);
					monUnite[elementsTires[m]].contenu = true;
					//On crée le décalage par rapport au centre du sac
					if(Math.pow(-1,nbEle)>0){
						var decalageX:Number = (1+nbEle%4)*Math.pow(-1,nbEle)*20;
						var decalageY:Number = 20-40*Math.floor(nbEle/4);
						monUnite[elementsTires[m]].setPosition(decalageX + fondMc["sac" + categorieDuSac]._x,
						decalageY + fondMc["sac" + categorieDuSac]._y);
					}else {
						var decalageX:Number = 20 + (1+nbEle%4)*Math.pow(-1,nbEle)*20;
						var decalageY:Number = 20-40*Math.floor(nbEle/4);
						monUnite[elementsTires[m]].setPosition(decalageX + fondMc["sac" + categorieDuSac]._x,
						decalageY + fondMc["sac" + categorieDuSac]._y);
					}
...
function creationBulle (clip,type,nomBulle,bouton, formatTexte)
{
	...
	clip[nomBulle].createTextField("repMcTxt",1,25,30,250,20);
...
finVert.onRelease=function()
{
	...
	fondMc.verifier_btn._visible = false;
	//On rend visible les sacPleins et invisible leur contenu
	for (noCat = 0; noCat < nbCategories; noCat++) {
		if(nbElementsSac[noCat]>0){
			fondMc["sac" + _level0.listeCategorie[noCat]]._visible=false;
			fondMc["sacPlein" + _level0.listeCategorie[noCat]]._visible = true;
			fondMc["sacPlein" + _level0.listeCategorie[noCat]].contenuNonBtn._visible = true;
			for (noEl = 0; noEl < tri.instances; noEl++) {
				if(monUnite[elementsTires[noEl]].categorieSac==_level0.listeCategorie[noCat]){
					monUnite[elementsTires[noEl]].visibilite(false);
				}
			}
		}
	}
...
{
	monUnite[retours[nbFautes - 1][2]].visibilite(false);
	monUnite[retours[nbFautes - 1][2]].active(true);
	monUnite[retours[nbFautes - 1][2]].setTaille(tailleClipPetit);
	monUnite[retours[nbFautes - 1][2]].setAlpha(50);
	monUnite[retours[nbFautes - 1][2]].setPosition(retours[nbFautes - 1][3], retours[nbFautes - 1][4]);
	//posNumero--;
	//On rend visible les sacPleins et invisible leur contenu
	for (noCat = 0; noCat < nbCategories; noCat++) {
		if(nbElementsSac[noCat]>0){
	...
	if(actionSolutionEnCours==false){
		fondMc.verifier_btn._visible = true;
	}
}

solutionBtn.onRelease = function() {
	actionSolution();
}
...
function afficherSuite()
{
	if(actionSolutionEnCours==false){
		...
		trace("afficherSuite, on n'est pas dans solution")
		fondMc.verifier_btn._visible = false;
		...
		if (nbFautesRestantes == 1) {
			trace("nbFautesRestantes == 1")
			rougeFinTxt.text = retours[nbFautes - nbFautesRestantes][0] + "\n\n\nCorrige maintenant.";
			//on remet le clip dans le sac
			monUnite[retours[nbFautes - nbFautesRestantes - 1][2]].visibilite(false);
			monUnite[retours[nbFautes - nbFautesRestantes - 1][2]].active(true);
			monUnite[retours[nbFautes - nbFautesRestantes - 1][2]].setTaille(tailleClipPetit);
			monUnite[retours[nbFautes - nbFautesRestantes - 1][2]].setAlpha(80);
			monUnite[retours[nbFautes - nbFautesRestantes - 1][2]].setPosition(retours[nbFautes - nbFautesRestantes - 1][3], retours[nbFautes - nbFautesRestantes - 1][4]);
			
			monUnite[retours[nbFautes - nbFautesRestantes][2]].visibilite(true);
			monUnite[retours[nbFautes - nbFautesRestantes][2]].active(true);
			monUnite[retours[nbFautes - nbFautesRestantes][2]].corrige = true;
			monUnite[retours[nbFautes - nbFautesRestantes][2]].contenu = false;
			monUnite[retours[nbFautes - nbFautesRestantes][2]].setPosition(posClipDepX, posClipDepY);
			monUnite[retours[nbFautes - nbFautesRestantes][2]].setTaille(monUnite[retours[nbFautes - nbFautesRestantes][2]].tailleDep);
			monUnite[retours[nbFautes - nbFautesRestantes][2]].setAlpha(100);
			...
			fondMc.verifier_btn._visible = false;
			nbFautesRestantes--;
		}else if (nbFautesRestantes < 1) {
			fondMc.verifier_btn._visible = true;
		}else {
			trace("dans else nbFautesRestantes != 1")
		...
		trace("afficherSuite, on est dans solution")
		fondMc.verifier_btn._visible = false;
		trace("nbFautes "+nbFautes);
		trace("nbFautesRestantes "+nbFautesRestantes);
		fondRS._visible = true;
		...
		if (nbFautesRestantes == 1) {
			trace("nbFautesRestantes == 1")
			rougeFinTxt.text = retoursSol[nbFautes - nbFautesRestantes][0] ;
			//on remet le clip dans le sac
			monUnite[retoursSol[nbFautes - nbFautesRestantes - 1][2]].visibilite(false);
			monUnite[retoursSol[nbFautes - nbFautesRestantes - 1][2]].active(true);
			monUnite[retoursSol[nbFautes - nbFautesRestantes - 1][2]].setTaille(tailleClipPetit);
			monUnite[retoursSol[nbFautes - nbFautesRestantes - 1][2]].setAlpha(80);
			monUnite[retoursSol[nbFautes - nbFautesRestantes - 1][2]].setPosition(retoursSol[nbFautes - nbFautesRestantes - 1][3], retoursSol[nbFautes - nbFautesRestantes - 1][4]);
			
			monUnite[retoursSol[nbFautes - nbFautesRestantes][2]].visibilite(true);
			monUnite[retoursSol[nbFautes - nbFautesRestantes][2]].active(true);
			monUnite[retoursSol[nbFautes - nbFautesRestantes][2]].corrige = true;
			monUnite[retoursSol[nbFautes - nbFautesRestantes][2]].contenu = false;
			monUnite[retoursSol[nbFautes - nbFautesRestantes][2]].setPosition(posClipDepX, posClipDepY);
			monUnite[retoursSol[nbFautes - nbFautesRestantes][2]].setTaille(monUnite[retoursSol[nbFautes - nbFautesRestantes][2]].tailleDep);
			monUnite[retoursSol[nbFautes - nbFautesRestantes][2]].setAlpha(100);
			...
			trace("dans else nbFautesRestantes != 1")
			rougeSuiteTxt.text = retoursSol[nbFautes - nbFautesRestantes][0];
			monUnite[retoursSol[nbFautes - nbFautesRestantes - 1][2]].visibilite(false);
			monUnite[retoursSol[nbFautes - nbFautesRestantes - 1][2]].active(true);
			monUnite[retoursSol[nbFautes - nbFautesRestantes - 1][2]].setTaille(tailleClipPetit);
			monUnite[retoursSol[nbFautes - nbFautesRestantes - 1][2]].setAlpha(80);
	...
	//trace("le .corrige du clip visible lorsqu'on a pressé sur suite " +monUnite[retours[nbFautes - nbFautesRestantes][2]].corrige)
}
...
oui.onRelease=function()
{
	...
	fondRON._visible = false;
	for (var i:Number = 0; i < sacObjet.length; i++) {
		if (sacObjet[i][0] == false) {
			monUnite[sacObjet[i][5]].corrige = true;
			monUnite[sacObjet[i][5]].contenu = false;
		}
		
	}
	if (nbFautesRestantes == 1) {
		trace("On est dans oui nbFautes restantes == 1")
		rougeFinTxt.text = retours[0][0]+"\n\n\nCorrige maintenant.";
		monUnite[retours[0][2]].visibilite(true);
		monUnite[retours[0][2]].active(true);
		monUnite[retours[0][2]].corrige = true;
		monUnite[retours[0][5]].contenu = false;
		monUnite[retours[0][2]].setPosition(posClipDepX, posClipDepY);
		
		monUnite[retours[0][2]].setTaille(monUnite[retours[0][2]].tailleDep);
		monUnite[retours[0][2]].setAlpha(100);
	...
	//trace("le .corrige du clip visible lorsqu'on a pressé sur oui " +monUnite[retours[nbFautes - nbFautesRestantes][2]].corrige)
}
