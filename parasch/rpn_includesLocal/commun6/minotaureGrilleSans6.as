...
for (i = 1; i <= nbCasesY; i++)
{
	...
	for (j = 1; j <= nbCasesX; j++)
	{
		...
		//on crée les cases
		maCase[nbTot] = new minotaureCase6(coteCase, rapportLongLarg, j, i, marge, true, fondMc["conteneurCarre" + nbTot], texteBas_fmt);
		...
		fondMc["conteneurCarre" + nbTot].onRollOver = function() 
		{
			...
			_level0.textCaseHaut=maCase[Number(this._name.substring(14))].conteneur.carre.val1.text;
			_level0.textCaseBas=maCase[Number(this._name.substring(14))].conteneur.carre.val2.text;
			...
			switch (true)
			{
				case(Number(this._name.substring(14))== departX):
					...
					maCase[departX].setTextB(textCaseBas, texteBas_fmt);
				...
				case(Number(this._name.substring(14))== noCase(arriveeX,arriveeY)):				
					...
					maCase[noCase(arriveeX,arriveeY)].setTextH(textCaseHaut, texteHaut_fmt);
				...
				case(fondMc["conteneurCarre" +Number(this._name.substring(14))].enabled==true && maCase[Number(this._name.substring(14))].active==true):				
					...
					maCase[Number(this._name.substring(14))].setTextH(textCaseHaut, texteHaut_fmt);
					maCase[Number(this._name.substring(14))].setTextB(textCaseBas, texteBas_fmt);
					break;
