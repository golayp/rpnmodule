...
//mouseListener.onMouseMove = function() {
	...
	//if(tempsVol[fuseau1][fuseau2][1] !=0 && tempsVol[fuseau1][fuseau2][0] != 0){
		//_global.rep_juste = analyse(fondMc.reponseM.text, fondMc.reponseH.text, decaM, decaH, tempsVol[fuseau1][fuseau2][1], tempsVol[fuseau1][fuseau2][0], monMenu3.posDansMenu);
	//}
	//trace ("maDestination1: " + maDestination1);
	...
	//_global.retour = maMontre.message;
}
...
function analyse(min:Number, h:Number,monDecalageM:Number, mondecalageH:Number, monTempsM:Number,monTempsH:Number, monJour:Number):Boolean {
	var minArr:Number = (maMinute + decaM + monTempsM) % 60;
	var hArr:Number = (monHeure + mondecalageH + monTempsH + Math.floor((maMinute + decaM + monTempsM) / 60)) % 24;
...
*/
	...
	var nbHeuresArr:Number = monHeure + mondecalageH + monTempsH + Math.floor((maMinute + decaM + monTempsM) / 60);
	...
	switch(true) {
		case (minArr == min && hArr == h && monJour == jourRep):
			...
			//actionSolution();
			_global.retour = "Bravo, on arrive bien "+jour[monJour-1]+"\nà "+hArr+"h "+minArr+"min.";
			break;
		case (minArr == min && hArr != h && monJour == jourRep):
		...
		case (minArr != min && hArr != h && monJour == jourRep):
			...
			break;
		case (minArr == min && hArr == h && monJour != jourRep):
			_global.retour = "le décalage des jours est faux";
			fondMc.felicitation._visible = false;
			break;
		case (minArr == min && hArr != h && monJour != jourRep):
			_global.retour = "les heures et le décalage des jours sont faux";
			fondMc.felicitation._visible = false;
			break;
		case (minArr != min && hArr == h && monJour != jourRep):
			_global.retour = "les minutes et le décalage des jours sont faux";
			fondMc.felicitation._visible = false;
			break;
		default:
			_global.retour = "Tout est faux.\nAs-tu bien séléctionné les lieux de déaprt et d'arrivée? ";
			fondMc.felicitation._visible = false;
			break;
	}
	trace("_global.retour: " + _global.retour)
	return monRetour;
}
