...
class dateHDec extends MovieClip
{
	...
	function dateHDec(XTemp,YTemp, clipConteneur, formatHTemp, formatMTemp, formatTxtTemp, formatTxtSolTemp,formatDateMontreTemp, tailleSceneTemp)
	{
	...
	function dessineLeClip()
	{
		...
		conteneur.conteneurTxtM.maxChars = 1;
	...
	*/
		...
		conteneur.conteneurTxtH._y = posY;
		conteneur.separateur._x = posX + 300;
		conteneur.separateur._y = posY;
		conteneur.conteneurTxtM._x = posX + 315;
		conteneur.conteneurTxtM._y = posY;
	...
	function visibiliteHetMin(choix:Boolean) {
		conteneur.conteneurTxtH._visible = choix;
		conteneur.conteneurTxtM._visible = choix;
	}
...
trace("")
	...
	function analyse(h, hAT, min, minAT):Boolean {
		minAT = Math.round(minAT / 6);
		//On modifie si les dixièmes à trouver sont 10 (par exemple pour 58min, ça fait 110 dixièmes donc 1h de plus et 0 dixièmes)
		if (minAT == 10) {
			minAT = 0;
			hAT = hAT + 1;
		}
		...
		case (h == hAT && min != minAT):
				rep = false;
				message = "Les dixièmes sont faux.";
		break;
		case (h != hAT && min == minAT):
			rep = false;
			if (h == hAT - 12) {
		...
		case (h != hAT && min != minAT):
			...
			if (h == hAT - 12) {
				...
				message = "Les heures et les dixièmes sont faux.";
			}
		break;
		}
	...
	function actionSolution(h, min) {
		...
		//posArr = deuxiemeBranche(posX, posY, angleValeurH, 5, 0x0ffff);//aiguille des heures
		//On modifie si les dixièmes à trouver sont 10 (par exemple pour 58min, ça fait 110 dixièmes donc 1h de plus et 0 dixièmes)
		if (Math.round(minuteATrouver / 6) == 10) {
			conteneur.conteneurTxtM.text = 0;
			conteneur.conteneurTxtH.text = heureATrouver + 1;
		}else{
			conteneur.conteneurTxtH.text = heureATrouver;
			conteneur.conteneurTxtM.text = Math.round(minuteATrouver / 6);
