...
class heureDAvion extends MovieClip
{
	...
	function heureDAvion(XTemp,YTemp, clipConteneur, formatHTemp, formatMTemp, formatTxtTemp, formatTxtSolTemp,formatDateMontreTemp, tailleSceneTemp)
	{
	...
	function dessineLeClip()
	{
		...
		trace("x"+conteneur.lampePocheMc._x)
		trace("Y"+conteneur.lampePocheMc._y)
...
//trace("deuxieme branche")
	...
	function setTime(h, m) {
		...
		heureDep = h;
		minDep = Math.round(m);//Sinon il existe un x.10-13 qui gène
		moisDep = my_dateGMT.getUTCMonth();
		dateDep = my_dateGMT.getUTCDate();
		trace(h+"h ;"+m+"min")
...
//trace("nbTours onEF"+o.nbTours)
				...
				//On met la nuit si on est avant 6h ou après 18h 
				/*if (monHeure >= 6 && monHeure < 18) {
					o.nuitVisible(false);
				}else if (monHeure >= 18 || monHeure < 6) {
					o.nuitVisible(true);
				}*/
				o.nuitVisibleHeure(monHeure, o.minutes);
	...
	/*function nuitVisible(valeur:Boolean) {
		if (valeur == true) {
			...
			conteneur.nuitMc._alpha = 0;
			conteneur.nuit2Mc._alpha = 0;
			conteneur.lampePocheMc._alpha = 0;
		
			for (var nuit = 0; nuit <= 1000;nuit++){
				conteneur.nuitMc._alpha = nuit/20;
				conteneur.nuit2Mc._alpha = nuit/20;
				conteneur.lampePocheMc._alpha = nuit/20;	
			}
		}else {
			for (var nuit = 0; nuit <= 1000;nuit++){
				conteneur.nuitMc._alpha = 50 - nuit / 10;
				conteneur.nuit2Mc._alpha = 50 - nuit / 10;
				conteneur.lampePocheMc._alpha = 50 - nuit / 10;	
			}
		}
		conteneur.nuitMc._visible = valeur;
		conteneur.nuit2Mc._visible = valeur;
	...
	function nuitVisibleHeure(h:Number, min:Number) {
		...
		//trace("NuitVisibleHeure heure minute: "+h+"h "+min+"min")
