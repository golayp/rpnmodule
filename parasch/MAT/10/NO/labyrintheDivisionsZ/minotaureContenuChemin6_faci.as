...
function resultatOperation(monOperation,m,n)
{
	switch (monOperation)
	{
		...
		case ("division"):
			if (n==0)
			{
				texteHaut_fmt.size=Math.floor((coteCase*2.1)/17);
				texteHaut_fmt.font="Arial Narrow";
				resultat="pas de\nsolution";
			}else {
				texteHaut_fmt.size=Math.floor((coteCase*3)/17);
				resultat=m/n;
			}
//			trace("m"+m);
//			trace("n"+n);
...
for (i = 0; i < longueurChemin+1; i++)
{
	...
	trace("choixOperation" + choixOperation);
	trace("tabHBJuste[posDepTab - i-1][0]" + tabHBJuste[posDepTab - i - 1][0]);
	trace("tabHBJuste[posDepTab - i - 1][1]" +tabHBJuste[posDepTab - i - 1][1] );
	...
	for (i = 1 ; i < nbCasesX+1 ; i++)
	{
		for (j = 1 ; j < nbCasesY+1 ; j++)
		{
			if (etatCase[i][j]=="")
			{
				...
				//trace("maCase[no].operationChoisie(choixOperation, tabHBJuste[noAlea - 1][0], tabHBJuste[noAlea - 1][1], texteBas_fmt)"+maCase[no].operationChoisie(choixOperation, tabHBJuste[noAlea - 1][0], tabHBJuste[noAlea - 1][1], texteBas_fmt));
			}
		}
	}
}
