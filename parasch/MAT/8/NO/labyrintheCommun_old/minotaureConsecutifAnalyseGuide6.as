	...
	switch (true)
	{
		...
		case(resultatAncienneCase==Number(maCase[numeroNouvelleCase].conteneur.carre.val1.text) )://Si on a cliqué sur un nb premier plus petit,dans l'ordre décroissant c'est juste. On met juste dans le tableau des réponses
			...
			if (numeroNouvelleCase == numeroArrivee)
			{
				...
				trace ("XpositionFauxG1: ");//+positionFauxG);
				trace ("positionFauxR1: "+positionFauxR);
				trace ("fin des positions1");
//				creationBulles();
...
//						}
		...
		break;
		case(resultatAncienneCase!=Number(maCase[numeroNouvelleCase].conteneur.carre.val1.text) )://Si on n'a pas pris le nombre premier consécutif
		compteurFaux++;
			trace("Faux");
			...
			if (numeroNouvelleCase == numeroArrivee)
			{
				...
				reperageFauxR(reponseChemin);
				reperageFauxG(reponseCheminGlobal);
				trace ("reponseChemin"+reponseChemin);
				trace ("reponseCheminGlobal"+reponseCheminGlobal);
				//On remet toutes les cases non selectionnables pour mettre seulement les bonnes 
				var nbDeCases=nbCasesX*nbCasesY;
