...
var noCaseArrivee : Number = noCase (arriveeX,arriveeY);
...
function construireChemin(depX, depY) {
	noEtape = 0;
	var etapeX : Number = depX;
	var etapeY : Number = depY;
		...
		for (i = 0; i < nbCasesX+1 ; i++)
		{
			for (j = 0; j < nbCasesY+1 ; j++)
			{
				if (etatCase[i][j]=="chemin")
				{
					...
					var monTextH:String = maCase[no].val1.text;
					var monTextB:String = maCase[no].val2.text;
					...
					maCase[no].setTextB(monTextB, texteHaut_fmt);
...
function construireImpasse()
{
	for (c = 0 ; c < longueurChemin ; c++)
	{
		...
		var no : Number = 0;
		var etapeX : Number = chemin[c][0];
		var etapeY : Number = chemin[c][1];
...
function construireImpasseDepuisFin()
{
	...
	var no : Number = 0;
	var etapeX : Number = arriveeX;
	var etapeY : Number = arriveeY;
