...
function fil(objetCase,type,orientation)
{
	...
	{
		case "chemin" :
		var couleurFil : Number = couleurFilChemin;
		var epaisseurFil : Number = epaisseurFilChemin;
		var no : Number = 2200 + noCase(posX, posY);
		var decalage:Number=0;
		break;
		case "parcours" :
		var couleurFil : Number = couleurFilParcours;
		var epaisseurFil : Number = epaisseurFilParcours;
		var no : Number = 1200 + noCase(posX, posY);
		var decalage:Number=5;
		break;
	}
	switch (orientation)
	{
	...
	switch (type)
	{
		...
		//trace("chemin");
			for(i=0;i<filSurChemin.length;i++)
			{
				...
				{
					//trace("this[filSurChemin[i]]._name.substring(4)"+this[filSurChemin[i]]._name.substring(this[filSurChemin[i]]._name.length-4));
		...
		case "parcours":
			for(i=0;i<filSurChemin.length;i++)
			{
				//trace("parcours"+this[filSurChemin[i]]._name.substring(this[filSurChemin[i]]._name.length-4));
