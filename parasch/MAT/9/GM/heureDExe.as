//C:\Users\luthi\Documents\JeanMichel\Paraschool\flash\heure\heureDExe.as
...
class heureDExe extends MovieClip
{
	...
	public var tailleM:Number;//taille de la montre
	...
	function heureDExe(XTemp,YTemp, clipConteneur, formatHTemp, formatMTemp, formatTxtTemp, formatTxtSolTemp,formatDateMontreTemp, tailleSceneTemp, tailleMontretemp)
	{
	...
	function dessineLeClip()
	{
		...
		//trace("formatDateMontre"+formatDateMontre)
		...
		comportement2(conteneur, tailleM);
...
//trace("deuxieme branche")
	...
	function premiereBranche(maTaille:Number, depX:Number, depY:Number, angleParRapportHorizontal:Number, epaisseur:Number, couleur):Array {//Dessine la branche puis retourne les coordonnées du départ du cercle
		...
		//trace("/////////////////////////////////////////////conteneur.angleMcM.branche1."+conteneur.angleMcM.branche1._visible)
		return coordPt;
	}
...
//trace("deuxieme branche")
	...
	function premiereBrancheP(maTaille:Number, depX:Number, depY:Number, angleParRapportHorizontal:Number, epaisseur:Number, couleur):Array {//Dessine la branche puis retourne les coordonnées du départ du cercle
		//On adapte la taille à la taille de l'aiguille (moitié du diamètre)
		maTaille = maTaille / 2;
		var coordPt:Array = new Array();
		var angleParRapportHorizontalRad:Number = degRad(angleParRapportHorizontal);
		...
		//trace("depX" + depX)
		//trace("depY" + depY)
		trace("maTailleP"+maTaille)
		conteneur.angleMcM.createEmptyMovieClip("branche1", 1);
		conteneur.angleMcM.branche1.lineStyle(epaisseur, couleur);
		conteneur.angleMcM.branche1.moveTo (depX - maTaille*1/55 * Math.cos(angleParRapportHorizontalRad), depY + maTaille*1/15 * Math.sin(angleParRapportHorizontalRad));
		conteneur.angleMcM.branche1.beginFill(couleur, 100);
		conteneur.angleMcM.branche1.lineTo (depX + maTaille*1/55 * Math.sin(angleParRapportHorizontalRad), depY + maTaille*1/15 * Math.cos(angleParRapportHorizontalRad));
		conteneur.angleMcM.branche1.lineTo (depX + maTaille*14/55 * Math.cos(angleParRapportHorizontalRad), depY - maTaille*14/15 * Math.sin(angleParRapportHorizontalRad));
		conteneur.angleMcM.branche1.lineTo (depX - maTaille* 1/55 * Math.sin(angleParRapportHorizontalRad), depY - maTaille*1/15 * Math.cos(angleParRapportHorizontalRad));
		conteneur.angleMcM.branche1.lineTo (depX -maTaille* 1/55 * Math.cos(angleParRapportHorizontalRad), depY + maTaille*1/15 * Math.sin(angleParRapportHorizontalRad));
		conteneur.angleMcM.branche1.endFill;
		coordPt[0] = posX + maTaille*14/25 * Math.cos(angleParRapportHorizontalRad);
		coordPt[1] = posY + maTaille*14/25 * Math.sin(angleParRapportHorizontalRad);
		//coordPt[2] = longueur;
		coordPt[3] = Math.cos(angleParRapportHorizontalRad);
		coordPt[4] = -Math.sin(angleParRapportHorizontalRad);
	...
	function deuxiemeBrancheP(maTaille:Number, depX:Number, depY:Number, angleParRapportHorizontal:Number, epaisseur:Number, couleur):Array {//Dessine la branche puis retourne les coordonnées de l'arrivée du cercle
		//On adapte la taille à la taille de l'aiguille (moitié du diamètre)
		maTaille = maTaille / 2;
		var coordPt:Array = new Array();
		//var longueur:Number;
...
//trace("deuxieme branche")
		conteneur.angleMcH.createEmptyMovieClip("branche2", 2);
		conteneur.angleMcH.branche2.lineStyle(epaisseur, couleur);
		conteneur.angleMcH.branche2.moveTo (depX - maTaille*1/50 * Math.cos(angleParRapportHorizontalRad), depY + maTaille*1/10 * Math.sin(angleParRapportHorizontalRad));
		conteneur.angleMcH.branche2.beginFill(couleur, 100);
		conteneur.angleMcH.branche2.lineTo (depX + maTaille*1/50 * Math.sin(angleParRapportHorizontalRad), depY + maTaille*1/10 * Math.cos(angleParRapportHorizontalRad));
		conteneur.angleMcH.branche2.lineTo (depX + maTaille*2/3 * Math.cos(angleParRapportHorizontalRad), depY - maTaille*2/3 * Math.sin(angleParRapportHorizontalRad));
		conteneur.angleMcH.branche2.lineTo (depX - maTaille*1/50 * Math.sin(angleParRapportHorizontalRad), depY - maTaille*1/10 * Math.cos(angleParRapportHorizontalRad));
		conteneur.angleMcH.branche2.lineTo (depX - maTaille*1/50 * Math.cos(angleParRapportHorizontalRad), depY + maTaille*1/10 * Math.sin(angleParRapportHorizontalRad));
		conteneur.angleMcH.branche2.endFill;
		coordPt[0] = posX + maTaille*2/3 * Math.cos(angleParRapportHorizontalRad);
		coordPt[1] = posY + maTaille*2/3 * Math.sin(angleParRapportHorizontalRad);
		//coordPt[2] = longueur;
	...
	function setTailleMontre(maTaille) {
		...
		formatDateMontre.size = maTaille / 15;
		trace("/////////////////////////////////////////////////////////////////////////////set<taille"+formatDateMontre.size)
		conteneur.angleMc0.jourTxt.setNewTextFormat(formatDateMontre);
		conteneur.angleMc0.moisTxt.setNewTextFormat(formatDateMontre);
		conteneur.angleMc0.jourTxt.text = dateDep;
		conteneur.angleMc0.moisTxt.text = moisTxtDep;
		...
		if (maTaille<100) {
			posDep = premiereBrancheP(maTaille, posX, posY, angleValeurM, 5, 0x00000);//Aiguille des minutes
			posArr = deuxiemeBrancheP(maTaille, posX, posY, angleValeurH, 5, 0x0ffff);//aiguille des heures
		}else{
	...
	function continu() {
		...
		//trace("posX" + posX)
		//trace("posY" + posY)
		//trace("angleValeurM" + angleValeurM)
		//trace("angleHeureMin"+angleHeureMin)
		posDep = premiereBranche(tailleM, posX, posY, angleValeurM, 5, 0x00000);//Aiguille des minutes
		posArr = deuxiemeBranche(tailleM, posX, posY, angleValeurH, 5, 0x0ffff);//aiguille des heures
		//trace("posDep" + posDep)
		//trace("posArr"+posArr)
	...
	function comportement2(clip, tailleDep) {
		var o = this;
		clip.onRollOver = function() {
			trace("ORO")
			o.setTailleMontre(200);
		}
		clip.onRollOut = function() {
			o.setTailleMontre(tailleDep);
