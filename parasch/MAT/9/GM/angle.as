...
private var plusPetitQue180:Boolean;
	...
	function angle(XTemp,YTemp, clipConteneur, formatTemp, typeAngleTemp, tailleSceneTemp)
	{
		posX = XTemp;
		posY = YTemp;
	...
	function dessineLeClip()
	{
		...
		//conteneur.angleMc.createEmptyMovieClip("branche1", 1);
		//conteneur.angleMc.createEmptyMovieClip("branche2", 2);
		//conteneur.angleMc.createEmptyMovieClip("remplissage", 3);
		...
		//dessineArcCercleInt(posX, posY, posDep[0], posDep[1],  posArr[0], posArr[1], 50, angleValeur, 2, 0xff0000, 0x00ff00, 50);
		
		//comportement(posX, posY);
		...
		//trace("angle: " + radDeg(angleEntreDeuxVecteurs(0, 2, 2, 2)));
	...
	function visibiliteNbDegres(choix:Boolean) {
		conteneur.conteneurTxt._visible = conteneur.conteneurTxtInfo._visible = choix;
		trace("visible? " +choix );
		trace("this.conteneurTxtInfo._visible? " +conteneur.conteneurTxtInfo._visible );
	}
	function premiereBranche(depX:Number, depY:Number, angleParRapportHorizontal:Number, epaisseur:Number, couleur):Array {//Dessine la branche puis retourne les coordonnées du départ du cercle
		...
		//trace("")
		//trace("Dessin de la première branche")
		//trace("angleParRapportHorizontal: "+angleParRapportHorizontal)
		...
		//	trace("Lcadran2")
		//	trace("Math.atan(depY / depX)"+Math.atan(depY / depX))
		...
		//	trace("Lcadran3")
		//	trace("2*Math.PI-angleParRapportHorizontal"+(2*Math.PI-angleParRapportHorizontalRad))
		...
		//conteneur.angleMc1.branche1.lineStyle(6, 0x0000ff);
		//conteneur.angleMc1.branche1.lineTo (depX + (longueurX /8)* Math.cos(angleParRapportHorizontal), depY + (hauteurY/8) * Math.sin(angleParRapportHorizontal));
	...
	function comportement( posx:Number, posy:Number)
	{
		//posx et posy sont les coordonnées du centre su cercle
		// Création d'un objet écouteur de souris et d'un écouteur de touche
		var mouseListener:Object = new Object();
		//Pour l'instant on n'utilise pas le listener sur le clavier
		var keyListener:Object = new Object();
		var o = this;
		var positionS:Array = new Array();
		var presse:Boolean = false;
		//On teste si la touche shift a été pressée
		keyListener.onKeyDown = function() {
			if (Key.isDown(Key.SHIFT)) {
				trace ("shift presse"+presse)
				switch (true) {
					case (presse == true):
						presse = false;
						break;
		...
		conteneur.angleMc1.onPress = function()
		{
			...
			mouseListener.onMouseMove = function() {
				positionS[0] = _level0._xmouse;
				positionS[1] = _level0._ymouse;
				//Calcul de l'angle en degrés entre 0 yc et 360
				switch (true) {
					//cadran 1 + 0°
					case(positionS[0] - posx >= 0 && positionS[1] - posy <= 0):
					//	trace("cadran1")
						o.angleValeur1 = o.radDeg(Math.abs(Math.atan((positionS[1] - posy) / (positionS[0] - posx))));
					...
					case(positionS[0] - posx <= 0 && positionS[1] - posy <= 0):
					//	trace("cadran2")
						o.angleValeur1 = o.radDeg(Math.PI - Math.abs(Math.atan((positionS[1] - posy) / (positionS[0] - posx))));
				...
				//		trace("branche 1 onPress cadran4")
						o.angleValeur1 = o.radDeg(2*Math.PI-Math.abs(Math.atan((positionS[1] - posy) / (positionS[0] - posx))));

						break;
					default:
						trace("hors cadran")
						trace("positionS[0] - posx" + (positionS[0] - posx))
						trace("positionS[1] - posy"+(positionS[1] - posy))
				}
				...
				o.dessineRemplissage(1, o.posX, o.posY, 150, o.angleVectTxt, o.posDep, o.posArr, 2, 0xffffff, 0xffffff, 100);
		...
		conteneur.angleMc2.onPress = function()
		{
			...
			mouseListener.onMouseMove = function() {
				...
				switch (true) {
					...
					case(positionS[0] - posx >= 0 && positionS[1] - posy <= 0):
					//	trace("cadran1")
						o.angleValeur2 = o.radDeg(Math.abs(Math.atan((positionS[1] - posy) / (positionS[0] - posx))));
						break;
					//90°
					case(positionS[0] - posx == 0 && positionS[1] - posy < 0):
					//	trace("90")
					...
					case(positionS[0] - posx <= 0 && positionS[1] - posy <= 0):
					//	trace("cadran2")
						o.angleValeur2 = o.radDeg(Math.PI - Math.abs(Math.atan((positionS[1] - posy) / (positionS[0] - posx))));
						break;
					//cadran 3
					case(positionS[0] - posx < 0 && positionS[1] - posy > 0):
					//	trace("cadran3")
					...
					//	trace("branche 2 cadran4")
						o.angleValeur2 = o.radDeg(2 * Math.PI - Math.abs(Math.atan((positionS[1] - posy) / (positionS[0] - posx))));
						break;
					default:
						trace("hors cadran")
						trace("positionS[0] - posx" + (positionS[0] - posx))
				...
				//trace("o.angleValeur dans branche2 onPress" + o.angleValeur)
				o.posDep = o.premiereBranche(posx, posy, o.angleValeur1 , 5, 0x000000);
				o.posArr = o.deuxiemeBranche(posx, posy, o.angleValeur2 , 5, 0x00ffff);

				
			//trace("o.angleValeur 2e branche" + o.angleValeur)
				...
				o.dessineRemplissage(2, o.posX, o.posY, 150, o.angleVectTxt, o.posDep, o.posArr, 2, 0xffffff, 0xffffff, 100);
				//trace("o.typeAngle"+o.typeAngle)
				o.reponse=o.analyse(o.typeAngle, o.angleVectTxt);
			}
	...
	function comportement2( posx:Number, posy:Number, positionSouris:Array)
	{
		//trace("comportement2")
		//posx et posy sont les coordonnées du centre su cercle
	...
	{
		//a1, a2, composante du premier vecteur
		//b1, b2, composante du 2e vecteur
		
		//trace("(a1 * b1 + a2 * b2 )" + ((a1 * b1 + a2 * b2 )))
		//trace("Math.sqrt(Math.pow(a1, 2) + Math.pow(a2, 2))" + (Math.sqrt(Math.pow(a1, 2) + Math.pow(a2, 2))))
		//trace("Math.sqrt(Math.pow(b1, 2) + Math.pow(b2, 2))" + (Math.sqrt(Math.pow(b1, 2) + Math.pow(b2, 2))))
		//trace("" + ())
	...
	function actionSolution(type) {
		switch(type){
			 case "vide":
				...
				dessineRemplissage(1, posX, posY, 150, 0, posDep, posArr, 2, 0xffffff, 0xffffff, 100);
			 ...
			 case "aigu":
				...
				dessineRemplissage(1, posX, posY, 150, 50, posDep, posArr, 2, 0xffffff, 0xffffff, 100);
			 ...
			 case "droit":
				...
				dessineRemplissage(1, posX, posY, 150, 90, posDep, posArr, 2, 0xffffff, 0xffffff, 100);
			 ...
			 case "obtus":
				...
				dessineRemplissage(1, posX, posY, 150, 140, posDep, posArr, 2, 0xffffff, 0xffffff, 100);
			 ...
			 case "plat":
				...
				dessineRemplissage(1, posX, posY, 150, 180, posDep, posArr, 2, 0xffffff, 0xffffff, 100);
			 ...
			 case "non convexe":
				...
				dessineRemplissage(1, posX, posY, 150, 235, posDep, posArr, 2, 0xffffff, 0xffffff, 100);
			 ...
			 case "plein":
				...
				dessineRemplissage(1, posX, posY, 150, 360, posDep, posArr, 2, 0xffffff, 0xffffff, 100);
