    ...
    var buildUi = function() {
        ...
        $.each($('b', domelem), function(idx, tofill) {
            ...
            if(ddmode){
                ...
                t.replaceWith($('<span>&#8239;<input type="text" class="rpnm_input gapsimple form-control">' + txt+'&#8239;</span>'));
    ...
    var validate = function(){
        if(ddmode){
            ...
            $.each($('.gapsimple',domelem), function(idx, gap) {
                //state[idx] = $(gap).val();
                state[idx] = $(gap).attr('name');
                rpnsequence.log('La réponse est:'+$(gap).attr('name'));
            });
        }
