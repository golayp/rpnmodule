...
var leftdiv, 
    ...
    var buildUi = function() {
        ...
        bezierlist[datas.idmodule]=new Array();
	...
	*/	
		$(document).ready(function(){
        	...
        	for (var i=0;i<nbbezier;i++){
                ...
                //var myId=i+10*Number(datas.idmodule);
                //rpnsequence.log('myId: '+myId);
                //rpnsequence.log('targets[datas.idmodule]'+targets[datas.idmodule][i].offset().top);
        	    ...
        	    bezierlist[datas.idmodule].push(bezier[i])
        	    nbbeziertotal++;
        	    rpnsequence.log('nbbeziertotal: '+nbbeziertotal);
        	    rpnsequence.log('bezierlist[datas.idmodule].length: '+bezierlist[datas.idmodule].length);
        	    rpnsequence.log('bezierlist.length: '+bezierlist.length);
    ...
    var score = function(sol) {
       ...
       // rpnsequence.log('bezier.target'+bezier[1].target)
        ...
        _.each(sol, function(item, ida){
            ...
            //for(i=0;i<=nbbeziertotal;i++){
     /*       _.each(bezierlist[ida], function(item, idx) {
               var myId=idx;
               rpnsequence.log('bezierlist[ida].length '+bezierlist[ida].length )
                //rpnsequence.log('ida num sol: '+ida)
                rpnsequence.log('idx: num bez: '+idx)
                //rpnsequence.log('myId: '+myId)
                rpnsequence.log('bezier[myId].target[0][0]: '+bezierlist[ida][myId].target[0][0]);
                rpnsequence.log('bezier[myId].target[1][0]: '+bezierlist[ida][myId].target[1][0]);
                
                if(typeof L_items[bezierlist[ida][myId].target[0][0]] !== 'undefined' &&
                    typeof R_items[bezierlist[ida][myId].target[1][0]] !== 'undefined' &&
                    typeof R_items[bezierlist[ida][myId].target[0][0]] !== 'undefined' &&
                    typeof L_items[bezierlist[ida][myId].target[1][0]] !== 'undefined' ){
                    rpnsequence.log('sol[ida][0]: '+sol[ida][0]);
                    rpnsequence.log('sol[ida][1]: '+sol[ida][1]);
                    rpnsequence.log('');
                    rpnsequence.log('L_items[bezier[myId].target[0][0]][0]: '+L_items[bezierlist[ida][myId].target[0][0]][0]);
                    rpnsequence.log('R_items[bezier[myId].target[1][0]][0]: '+R_items[bezierlist[ida][myId].target[1][0]][0]);
                    rpnsequence.log('R_items[bezier[myId].target[0][0]][0]: '+R_items[bezierlist[ida][myId].target[0][0]][0]);
                    rpnsequence.log('L_items[bezier[myId].target[1][0]][0]: '+L_items[bezierlist[ida][myId].target[1][0]][0]);
                }
                ...
                score+=(
                        (
                            (typeof L_items[bezierlist[ida][myId].target[0][0]] !== 'undefined' &&
                            typeof R_items[bezierlist[ida][myId].target[1][0]] !== 'undefined' &&
                            typeof R_items[bezierlist[ida][myId].target[0][0]] !== 'undefined' &&
                            typeof L_items[bezierlist[ida][myId].target[1][0]] !== 'undefined' ) &&
                        (
                            (L_items[bezierlist[ida][myId].target[0][0]][0]==sol[ida][0] && R_items[bezierlist[ida][myId].target[1][0]][0]==sol[ida][1])||
                            (L_items[bezierlist[ida][myId].target[1][0]][0]==sol[ida][0] && R_items[bezierlist[ida][myId].target[0][0]][0]==sol[ida][1])
                        )
                    )?1:0);
               //score++;
